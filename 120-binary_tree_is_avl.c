 ...
 * @tree: a pointer to the tree's root node that measures the height.
 * Return: function return 0 if tree is NULL; else, it return height.
 */
size_t height(const binary_tree_t *tree)
{
	if (tree)
...
/**
 * is_avl_helper - a binary tree is a valid AVL tree.
 * @tree: a pointer to the tree's root node for checking.
 * @lo: value of the smallest node so far visited.
 * @hi: value of the highest node so far visited.
 * Return: 1 if the tree a valid AVL tree; otherwise, 0.
 */
int is_avl_helper(const binary_tree_t *tree, int lo, int hi)
{
